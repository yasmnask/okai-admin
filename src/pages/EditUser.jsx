...
import { 
  ArrowLeft, Save, Mail, ShieldCheck, 
  Lock, User, Info, Loader2
} from 'lucide-react';
    ...
    const loadUser = async () => {
      try {
        ...
        if (response.success) {
          ...
          if (userData.role === 'superadmin' || userData.role === 'affiliate') {
            alert('Akses Ditolak: Anda tidak memiliki izin untuk mengedit profil ini.');
            navigate('/users');
            return;
          }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if(response.success) {
        ...
        alert("Gagal menyimpan: " + JSON.stringify(response.errors || "Periksa kembali data Anda"));
      }
    } catch (error) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10 sticky top-0 z-10 bg-[#F8FAFC]/80 backdrop-blur-md py-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-2xl font-black text-[#1E293B]">Edit <span className="text-[#E65100]">User</span></h1>
          </div>
        </div>
        ...
        <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex justify-between">
                  ...
                  <span className="text-orange-400 italic font-medium">(Opsional)</span>
                </label>
                <div className="relative">
            ...
            <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 flex gap-4 items-start mt-6">
              ...
              <div>
                <p className="text-xs font-black text-slate-700 uppercase tracking-tight mb-1">Informasi Pembaruan</p>
                <p className="text-[11px] text-slate-500 leading-relaxed font-medium">
                  Pastikan alamat email yang dimasukkan valid dan belum digunakan oleh pengguna lain. Jika kolom kata sandi dikosongkan, kata sandi lama akan tetap dipertahankan.
                </p>
              </div>
            </div>
          </form>
