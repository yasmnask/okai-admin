...
export default function ProfileSettings() {
  ...
  const handleSubmit = async () => {
    ...
    if (!isAllowed) {
      alert(
        "Akses Ditolak: Fitur ubah profil ini hanya diperuntukkan bagi Admin Operasional.",
      );
      return;
    }
  ...
  return (
    <div className="p-8 bg-[#F8FAFC] dark:bg-[#1a1e1a] min-h-screen">
      <div className="max-w-2xl mx-auto">
        <h1 className="text-2xl font-black mb-2 text-[#1E293B] dark:text-white">
          Profile <span className="text-[#E65100]">Settings</span>
        </h1>
        <p className="text-slate-400 dark:text-[#e1d4cc] text-sm mb-10 font-medium italic">
        ...
        {!isAllowed && (
          <div className="mb-6 p-4 bg-red-50 border border-red-100 rounded-2xl flex items-center gap-3 text-red-600">
            ...
            <p className="text-sm font-bold">
              Mode Baca Saja: Hanya role Admin yang diizinkan untuk mengubah
              data di halaman ini.
            </p>
          </div>
        )}
        ...
        <div className="space-y-6">
          {/* Informasi Profil */}
          <div className="bg-white dark:bg-[#1a1d1a] p-8 rounded-[2.5rem] border border-slate-100 dark:border-0 dark:shadow-black dark:shadow-xl shadow-sm">
            <h3 className="flex items-center gap-2 font-bold text-slate-800 dark:text-slate-200 mb-6">
              <User size={18} className="text-[#E65100]" /> Data Diri
            </h3>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-black text-slate-400 dark:text-[#e1d4cc] uppercase tracking-widest mb-2 ml-1">
                  Nama Lengkap
                </label>
                <div className="relative">
                  <User
                    className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-100"
                    size={16}
                  />
                  <input
                    type="text"
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-slate-50 dark:bg-[#3e3c3a] border-none rounded-2xl text-slate-500 dark:text-slate-100 text-sm focus:ring-2 focus:ring-orange-500/20 dark:focus:ring-[#E65100] outline-none font-medium disabled:opacity-50"
                    placeholder="Masukkan nama lengkap Anda..."
                  />
                </div>
              </div>

              {/* Kolom Email (Ditampilkan sebagai Read Only) */}
              <div>
                <label className="block text-xs font-black text-slate-400 dark:text-[#e1d4cc] uppercase tracking-widest mb-2 ml-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    disabled
                    className="w-full pl-11 pr-4 py-3 bg-slate-100 dark:bg-[#3e3c3a] border-none rounded-2xl text-sm outline-none font-medium text-slate-500 dark:text-slate-100 cursor-not-allowed"
                  />
                </div>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-[#1a1d1a] p-8 rounded-[2.5rem] border border-slate-100 dark:border-0 dark:shadow-black dark:shadow-xl shadow-sm">
            <h3 className="flex items-center gap-2 font-bold text-slate-800 dark:text-slate-200 mb-6">
              <ShieldCheck size={18} className="text-[#E65100]" /> Ganti
              Password
            </h3>
            <p className="text-xs text-slate-400 dark:text-[#e1d4cc] mb-4 font-medium italic">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input
                  ...
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-[#3e3c3a] border-none rounded-2xl text-slate-500 dark:text-slate-100 text-sm focus:ring-2 focus:ring-orange-500/20 dark:focus:ring-[#E65100] outline-none disabled:opacity-50"
                />
                <input
                  type="password"
